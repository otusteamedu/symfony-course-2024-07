...
21. В контейнере запускаем консьюмер командой `php bin/console rabbitmq:consumer add_followers -m 1000`
22. Выполняем запрос Post tweet из Postman-коллекции v9 с параметром `async` = 0, проверяем, что ленты материализовались
